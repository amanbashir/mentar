...
export const getAIChatResponse = async (
  ...
  currentProject: any,
  systemPrompt: string,
  useGPT4: boolean = true // Add model selection parameter
) => {
  try {
    // Input validation
    if (!currentProject) {
...
- Using Model: ${useGPT4 ? 'GPT-4' : 'GPT-3.5'}
...
5. DO show all work and results`
    ...
    const truncatedMessages = truncateMessageHistory(messages, useGPT4 ? 6000 : 3000);
...
export async function generateTodosForStage(stage: StageKey, currentProject: any) {
  ...
  try {
    ...
    const blueprint = getStrategyForBusinessType(business_type);

    // Verify that the stage exists in the blueprint
    if (!blueprint[stage]) {
      console.error(`Stage ${stage} not found in ${business_type} blueprint`);
      return null;
    }
    ...
    // Add additional checklist items to prevent image-related tasks
    const additionalChecklist = [
      "DO NOT include any tasks related to logo design or generation",
      "DO NOT include any tasks related to image creation or editing",
      "DO NOT include any tasks related to banner or visual asset creation",
      "DO NOT include any tasks related to icon design",
      "DO NOT include any tasks related to AI image generation",
      "Focus on business operations and strategy",
      "Focus on measurable outcomes and metrics",
      "Focus on technical setup and automation",
      "Focus on customer acquisition and retention",
      "Focus on analytics and tracking implementation"
    ];

    // Add these to the stage data
    if (!stageData.checklist) {
      stageData.checklist = [];
    }
    stageData.checklist = [...additionalChecklist, ...stageData.checklist];

    // Create a detailed prompt for the current stage
    const todoGenerationPrompt = `As an AI ${business_type} business mentor, generate specific, actionable tasks for stage ${stage} of building a ${business_type} business.

STAGE OBJECTIVE: ${stageData.objective}

CURRENT STAGE DETAILS:
${stageData.steps?.map((step: { title: string; goal?: string }) => `- ${step.title}: ${step.goal || ''}`).join('\n')}

CHECKLIST ITEMS:
${stageData.checklist.map((item: string) => `- ${item}`).join('\n')}

DELIVERABLES:
${stageData.deliverables?.map((item: string) => `- ${item}`).join('\n') || 'No specific deliverables defined'}

IMPORTANT GUIDELINES:
1. DO NOT include any tasks related to:
   - Logo design or generation
   - Image creation or editing
   - Banner or visual asset creation
   - Icon design
   - Any AI image generation
   Instead, assume the user will handle visual assets separately.

2. Each task MUST be:
   - Ultra-specific with exact steps
   - Immediately actionable
   - Measurable with clear completion criteria
   - Focused on business impact
   - Independent of visual asset creation

3. Include for each task:
   - Specific tools or platforms to use (with URLs)
...
- Budget: ${budget || "0"}
- Stage: ${stage}
- Stage Progress: Stage ${stage.replace('stage_', '')} of the blueprint
...
[
  {
    "task": "Set up conversion tracking in Google Analytics 4 by visiting analytics.google.com, creating a new GA4 property, adding the tracking code to your website header, and configuring 3 key conversion events: signup, purchase, and email subscription. Test each event using the real-time analytics view.",
    "completed": false
  }
]
...
- Tasks should follow a logical progression
- Each task should take 1-4 hours to complete`;
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.error("OpenAI API Error in generateTodosForStage:", {
        status: response.status,
        statusText: response.statusText,
        errorData,
      });
