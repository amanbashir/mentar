...
export const handler = async (event) => {
  console.log(
    `Function invoked with method: ${event.httpMethod}, path: ${event.path}`
  );
  ...
  try {
    ...
    if (
      ...
      priceId === "price_1RTr8L76vITl07GqpgH91QTy"
    ) {
      actualPriceId = activePrices.pro;
    } else if (
      priceId.includes("business") ||
      priceId === "price_1RTr8g76vITl07GqL9frQYbF"
    ) {
    ...
    if (actualPriceId !== priceId) {
      console.log(
        `Mapped price ID ${priceId} to actual price ID ${actualPriceId}`
      );
    }
    ...
    try {
      ...
      console.error(
        `Price ${actualPriceId} does not exist, creating new prices`
      );
